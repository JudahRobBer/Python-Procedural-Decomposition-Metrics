...
#plant function to calculate the plants needed for the garden(total, semi-circle, circle)
def plant(side_len: float, plant_spacing:float)-> int:
    pi = 3.14
...
# fill function to calculate the area needed to be filled in cubic yards

def fill_yds(side_len: float, circle_area: float, fill_depth:float)->None:
...
# soil function to calculate the amount of soil needed for the garden (total, semi-circle, circle)
def soil(soil_depth:float, circle_area:float)->None:
...
#main function used as entry point to the program and puts the other functions together
def main()->None:
    ...
    circle_area = plant(side_len,plant_spacing)
    soil(soil_depth, circle_area)
    fill_yds(side_len,circle_area,fill_depth)
