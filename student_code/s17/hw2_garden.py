...
def fill(circle_area: float, side_len: float, fill_depth: float) -> None:
...
def plants(circle_area: float, plant_spacing: float, side_len: float) -> None:
...
def main() -> None:
    ...
    plants(circle_area, plant_spacing, side_len)
    soil(circle_area, soil_depth)
    fill(circle_area, side_len, fill_depth)
